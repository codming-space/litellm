        ...
        for c in content_list:
            if isinstance(c, str):
                ...
                if isinstance(tool_result_content, str):
                    num_tokens += count_function(tool_result_content)
                ...
                # Handle tool_use content (Anthropic format)
                if "name" in c:
                    num_tokens += count_function(c["name"])
                if "input" in c:
                    num_tokens += count_function(str(c["input"]))
            else:
                raise ValueError(
